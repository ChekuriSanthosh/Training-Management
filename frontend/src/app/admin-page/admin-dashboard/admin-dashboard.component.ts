...
export class AdminDashboardComponent implements OnInit{
  ...
  async fetchDashboardData() {
    try {
      ...
      const students = await this.http.get<any[]>('http://localhost:8888/admin/getAllStudents').toPromise();
      this.dashboardData.unassignedStudents = students?.filter(s => !s.trainingRoom).length || 0;

      
      const roomCount = await this.http.get<number>('http://localhost:8888/admin/GetTrainingRoomSize').toPromise() || 0;
      for (let i = 1; i <= roomCount; i++) {
        const roomStudents = await this.http.get<any[]>(`http://localhost:8888/admin/students/Room${i}`).toPromise();
        this.studentsdata=roomStudents;
        this.dashboardData.studentsPerRoom[`Room${i}`] = roomStudents?.length || 0;
      }

      
      const requests = await this.http.get<any[]>('http://localhost:8888/admin/request').toPromise();
      this.dashboardData.pendingRequests = requests?.filter(r => r.status === 'PENDING').length || 0;

      
      const trainers = await this.http.get<any[]>('http://localhost:8888/admin/trainers').toPromise();
  ...
  async onRoomChange() {
    ...
    try {
      const students = await this.http.get<any[]>(`http://localhost:8888/admin/students/${this.selectedRoom}`).toPromise();
  ...
  private async createMarksChart() {
    try {
      ...
      for (const room of this.availableRooms) {
        const students = await this.http.get<any[]>(`http://localhost:8888/admin/students/${room}`).toPromise();
